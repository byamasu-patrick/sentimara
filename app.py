...
from chat.pg_vector import get_vector_store_singleton, CustomPGVectorStore

load_dotenv()

OPEN_API_JMF_LLM_KEY = os.getenv('OPEN_API_JMF_LLM_KEY')
...
async def lifespan(app: FastAPI):
    ...
    if not check_current_head(cfg, engine):
        raise Exception(
            "Database is not up to date. Please run `alembic upgrade head`"
        )
