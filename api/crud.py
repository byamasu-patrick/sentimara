...
from libs.models.chatdb import (Conversation, ConversationDocument, Document,
                                HumanFeedback, Message)
...
async def update_conversation_headline(
    ...
    stmt = (
        ...
        .options(
            joinedload(Conversation.conversation_documents)
        )
    ...
    if conversation is not None:
        ...
        print(headline)
        conversation.headline = headline
        await db.commit()
...
def find_previous_message(messages: List[schema.Message], target_id: str) -> Optional[schema.Message]:
    prev_message = None
...
async def create_human_feedback(
    ...
    human_feedback.assistant_message_id = human_feedback_data.assistant_message_id
    human_feedback.is_good_response = human_feedback_data.is_good_response
    human_feedback.conversation_id = human_feedback_data.conversation_id

    conversation: schema.Conversation = await fetch_conversation_with_messages(db, str(human_feedback_data.conversation_id))
    messages: List[schema.Message] = conversation.messages

    prev_message: schema.Message = find_previous_message(
        messages=messages, target_id=human_feedback_data.assistant_message_id)

    if prev_message:
        human_feedback.user_message_id = prev_message.id
    ...
    return schema.HumanFeedbackResponse(**human_feedback_dict)


async def update_human_feedback(
    db: AsyncSession,
    human_feedback_data: schema.HumanFeedbackUpdate
) -> Optional[schema.HumanFeedbackResponse]:
