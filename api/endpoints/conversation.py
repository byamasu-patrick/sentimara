...
async def create_conversation(
    ...
    documents: Sequence[schema.Document] = await crud.fetch_documents(db=db,
                                                                      table_names=payload.document_ids)
    # print(documents)
    for document in documents:
        document_payload.document_ids.append(document.id)
...
async def create_human_feedback(
    ...
    """
    response: schema.HumanFeedbackResponse = await crud.create_human_feedback(db=db,
                                                                              human_feedback_data=payload)
...
async def update_human_feedback(
    ...
    if human_feedback_id == payload.id:
        response: schema.HumanFeedbackResponse = await crud.update_human_feedback(db=db,
                                                                                  human_feedback_data=payload)
        return response
    ...
    async def event_publisher():
        async with send_chan:
            task = asyncio.create_task(
                handle_chat_message(conversation, user_message,
                                    send_chan, temperature)
            )
...
async def test_message_conversation(
    ...
    response: EventSourceResponse = await message_conversation(
        conversation_id, user_message, temperature, db,
    )
    final_message = None
