...
def upsert_respondent(pg_session, sqlite_session, id: int) -> None:
    ...
    sql_query = text("SELECT * FROM pi_survey_respondents WHERE id=:respondent_id")

    # Retrieve data from pi_survey_respondents table
    pg_respondents = pg_session.execute(sql_query, {"respondent_id": id}).fetchall()
...
def upsert_responses(pg_session, sqlite_session, id: int) -> None:
    ...
    # Retrieve data from pi_survey_respondents table
    pg_responses = pg_session.execute(sql_query, {"response_id": id}).fetchall()
...
def upsert_into_sqlite_respondent(session, data):
    ...
    if respondent:
        ...
        respondent.mobile_international_dialing_code = (
            data.mobile_international_dialing_code
        )
...
def upsert_progresses(pg_session, sqlite_session, id: int):
    ...
    # Retrieve data from pi_survey_respondents table
    pg_progresses = pg_session.execute(sql_query, {"progress_id": id}).fetchall()
...
def upsert_treatment_patterns(pg_session, sqlite_session, id: int):
    ...
    # Retrieve data from pi_survey_respondents table
    pg_patterns = pg_session.execute(sql_query, {"pattern_id": id}).fetchall()
...
def upsert_into_sqlite_pattern(session, data):
    pattern = session.query(TreatmentPattern).filter_by(id=data.id).first()
    ...
    if pattern:
        ...
        pattern.patients_receiving_ig_g_ivig_clinic = (
            data.patients_receiving_ig_g_ivig_clinic
        )
        pattern.patients_receiving_ig_g_ivig_home = (
            data.patients_receiving_ig_g_ivig_home
        )
        ...
        pattern.patients_treated_with_gene_therapy = (
            data.patients_treated_with_gene_therapy
        )
        pattern.patients_treated_with_peg_ada = data.patients_treated_with_peg_ada
        pattern.patients_treated_by_transplant_donor_type_mrd = (
            data.patients_treated_by_transplant_donor_type_mrd
        )
        pattern.patients_treated_by_transplant_donor_type_mud = (
            data.patients_treated_by_transplant_donor_type_mud
        )
        pattern.patients_treated_by_transplant_donor_type_m_mud = (
            data.patients_treated_by_transplant_donor_type_m_mud
        )
        pattern.patients_treated_by_transplant_donor_type_haplo = (
            data.patients_treated_by_transplant_donor_type_haplo
        )
        pattern.patients_treated_by_transplant_stem_cell_src_bm = (
            data.patients_treated_by_transplant_stem_cell_src_bm
        )
        pattern.patients_treated_by_transplant_stem_cell_src_pbsc = (
            data.patients_treated_by_transplant_stem_cell_src_pbsc
        )
        pattern.patients_treated_by_transplant_stem_cell_src_cord = (
            data.patients_treated_by_transplant_stem_cell_src_cord
        )
        pattern.patients_treated_by_transplant_stem_cell_src_other_name = (
            data.patients_treated_by_transplant_stem_cell_src_other_name
        )
        pattern.patients_treated_by_transplant_stem_cell_src_other_count = (
            data.patients_treated_by_transplant_stem_cell_src_other_count
        )
        pattern.created_at = data.created_at
        pattern.updated_at = data.updated_at
        pattern.published_at = data.published_at
        pattern.patients_diagnosed_through_jeffrey_insights_program = (
            data.patients_diagnosed_through_jeffrey_insights_program
        )
        # Commit the changes
        session.commit()
...
def upsert_respondent_logins(pg_session, sqlite_session, id: int):
    ...
    # Retrieve data from pi_survey_respondents table
    pg_logins = pg_session.execute(sql_query, {"login_id": id}).fetchall()
...
def upsert_into_sqlite_login(session, data):
    login = session.query(RespondentLogin).filter_by(id=data.id).first()
...
def upsert_completions(pg_session, sqlite_session, id: int):
    ...
    # Retrieve data from pi_survey_respondents table
    pg_completions = pg_session.execute(sql_query, {"completion_id": id}).fetchall()
...
def upsert_into_sqlite_completion(session, data):
    completed = session.query(CompletedSurveyRespondents).filter_by(id=data.id).first()
...
def upsert_net_promoter_scores(pg_session, sqlite_session, id: int):
    ...
    # Retrieve data from pi_survey_respondents table
    pg_promoters = pg_session.execute(sql_query, {"promoter_id": id}).fetchall()
...
def upsert_into_sqlite_promoter(session, data):
    promoter = session.query(NetPromoterScore).filter_by(id=data.id).first()
...
def upsert_demographic_responses(pg_session, sqlite_session, id: int):
    ...
    # Retrieve data from pi_survey_respondents table
    pg_demographics = pg_session.execute(sql_query, {"demographic_id": id}).fetchall()
...
def upsert_into_sqlite_demographic(session, data):
    demographic = session.query(PatientDemographic).filter_by(id=data.id).first()
