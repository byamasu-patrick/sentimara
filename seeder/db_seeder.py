...
class DBSeeder:
    ...
    def insert_into_sqlite_respondent(self, pg_respondents):
        for data in pg_respondents:
            existing_respondent = (
                self.sqlite_session.query(Respondent).filter_by(id=data.id).first()
            )
    ...
    def insert_into_sqlite_response(self, pg_responses, batch_size=100):
        ...
        for pg_response in pg_responses:
            existing_response = (
                self.sqlite_session.query(Response).filter_by(id=pg_response.id).first()
            )
            ...
            if pg_response.patient_reported.lower() != "nan":
                ...
                if patient_reported_int > 0:
                    new_response = Response(
                        ...
                        job_title=pg_response.job_title,
    ...
    def insert_into_sqlite_treatment_patterns(self, pg_treatment_patterns):
        ...
        for pg_treatment_pattern in pg_treatment_patterns:
            existing_treatment_pattern = (
                self.sqlite_session.query(TreatmentPattern)
                .filter_by(id=pg_treatment_pattern.id)
                .first()
            )
    ...
    def insert_into_sqlite_demographic(self, pg_demographics):
        ...
        for pg_demographic in pg_demographics:
            existing_demographic = (
                self.sqlite_session.query(Demographic)
                .filter_by(id=pg_demographic.id)
                .first()
            )
    ...
    def insert_into_sqlite_completion(self, pg_completions):
        ...
        for pg_completion in pg_completions:
            existing_completion = (
                self.sqlite_session.query(CompletedSurvey)
                .filter_by(id=pg_completion.id)
                .first()
            )
    ...
    def insert_into_sqlite_colleague(self, pg_colleagues):
        ...
        for pg_colleague in pg_colleagues:
            existing_colleague = (
                self.sqlite_session.query(Colleagues)
                .filter_by(id=pg_colleague.id)
                .first()
            )
    ...
    def insert_into_sqlite_proxy_respondent(self, pg_proxy_respondents):
        ...
        for pg_proxy_respondent in pg_proxy_respondents:
            existing_proxy_respondent = (
                self.sqlite_session.query(ProxyRespondent)
                .filter_by(id=pg_proxy_respondent.id)
                .first()
            )
    ...
    def insert_into_sqlite_net_promoter_score(self, pg_net_promoter_scores):
        ...
        for pg_net_promoter_score in pg_net_promoter_scores:
            existing_promoter = (
                self.sqlite_session.query(NetPromoterScore)
                .filter_by(id=pg_net_promoter_score.id)
                .first()
            )
    ...
    def fetch_survey_data(self):
        ...
        return (
            pg_respondents,
            pg_responses,
            pg_treatment_patterns,
            pg_demographics,
            pg_colleagues,
            pg_proxy_respondents,
            pg_completions,
            pg_net_promoter_scores,
        )
