...
DEFAULT_ORCHESTRATOR_PROMPT = (
    "You are on an orchestration agent.\n"
    "Your job is to decide which agent order the order of the agents to run based on the current state of the user and what they've asked to do.\n"
    "You do need to figure out dependencies between agents; for example marketing agent will sometimes require results from PI Survey Data Analyst (7-Table Scope) agent and Geospatial PI Defect Epidemiology Analyst for it to achieve its goal.\n"
    "Here are the agents you can choose from:\n{agent_context_str}\n\n"
    "Here is the current user state:\n{user_state_str}\n\n"
    "Please assist the user and transfer them as needed.\n\n"
    "Important: Kindly use the results from subquestion query engines to refine the final results. If no results are found, suggest that they may refine their prompt to make it more specific or concise for better results. For example, they might want to focus on specific data points or include more context."
)
...
class ConciergeAgent(Workflow):
    ...
    async def setup(
        ...
        llm: LLM = ev.get("llm", default=CustomSettings.llm)
        # llm = CustomSettings.llm
