...
def get_custom_response_synthesis_for_responses_table(
    callback_manager: Optional[CallbackManager] = None,
) -> BaseSynthesizer:
    """
    Creates and returns a custom response synthesizer for handling user queries about the 'responses' table in the context of Primary Immunodeficiency (PI) data analysis.

    This function sets up response synthesizers with specialized templates for refining existing answers
    and generating new answers to user queries. The templates include a list of database table names
    and placeholders for query context, original queries, and existing answers (if any). This setup
    enables more contextual and accurate responses to queries related to the 'responses' table.
    ...
    Returns:
    BaseSynthesizer: An instance of a response synthesizer configured with custom prompt templates and settings for handling queries on the 'responses' table.
    """
    table_names = "\n".join("- " + table_name for table_name in ['responses'])

    refine_template_str = f"""
      A user has a 'responses' table and has asked a question about it. \
      ...
      # Note: Do not return 'It seems that there is no specific data available' if you have enough context to refine the answer
